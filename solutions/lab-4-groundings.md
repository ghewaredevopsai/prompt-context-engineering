...
|---|--:|--:|
| A: attach everything | ~46,700 | 78x |
| B: `rating.py` + `manifest.py` | ~2,900 | 4.8x |
| C: six pasted runbook lines + the failing test output | ~600 | 1x |
...
It is not that the model got confused by volume, although 46,700 tokens of mostly-tariff-data does
not help.
...
the measurement that proves it: sixteen times the context, and a worse answer, because one of the
